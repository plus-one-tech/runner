...
ヘッダは `.run` ファイルにおいて必須とする。
ヘッダが存在しない `.run` は不正なフォーマットとして扱う。

`.run` は実行方法を内包するファイルであり、
先頭ヘッダによって runtime または実行モードを決定する。

`.run` 以外のファイルで、`runner.env` の `ext.<extension>` により
runtime を解決できるものは、ヘッダを必要としない。
...
runtime の解決は `runner.env` の以下の定義に従う。
